...
const isClientUnlocked = computed(() => {
  return Boolean(isNetworkOnline.value && isServerReachable.value && authToken.value);
});
...
| 登录弹窗样式
|--------------------------------------------------------------------------
| 强制访问弹窗使用 Codex 风格深灰浮层，确保未登录时无法操作主界面。
|--------------------------------------------------------------------------
*/
